...
const Dot = styled.span`
  ...
  border-radius: 50%;
  background-color: ${(props) =>
    props.color === 'green' ? '#28a745' : '#dc3545'};
`;
  ...
  const getSoldRecord = async () => {
    try {
      ...
      const response = await api.post(
        `/api/Purchase/customer-sold-record/${customerNumber}`,
        { imei: payload.imei }
      );
      ...
      if (responseData) {
        ...
        const allRecords = [
          ...(responseData.singlePurchases || []).map((record) => ({
            ...record,
            type: 'purchase',
          })),
          ...(responseData.singleSales || []).map((record) => ({
            ...record,
            type: 'sold',
          })),
          ...(responseData.bulkPurchases || []).map((record) => ({
            ...record,
            type: 'purchase',
          })),
          ...(responseData.bulkSales || []).map((record) => ({
            ...record,
            type: 'sold',
          })),
        ];
    ...
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      <div
        style={{
          ...
          borderRadius: '16px',
          background:
            'linear-gradient(135deg, #6366f1 0%, #8b5cf6 40%, #14b8a6 100%)',
        ...
        >
          ...
          >
            <div>
              ...
              >
                Search by phone number or IMEI to view a customer's purchase and
                sales history.
              </p>
            </div>
            <div
          ...
          <div style={{ display: 'flex', gap: '10px', marginBottom: '14px' }}>
            ...
            <button
              ...
              style={{
                ...
                borderRadius: '10px',
                border:
                  idType === 'imei' ? '2px solid #6366f1' : '1px solid #e5e7eb',
            ...
            <button
              ...
              onMouseOver={(e) => {
                ...
                e.currentTarget.style.boxShadow =
                  '0 14px 22px rgba(99,102,241,0.3)';
              }}
              onMouseOut={(e) => {
                ...
                return (
                  <Card key={doc._id || index} type="sold">
                    ...
                    <InfoGrid>
                      {doc.customerName && (
                        <InfoItem>
                          <strong>Customer Name:</strong> {doc.customerName}
                        </InfoItem>
                      )}
                      {doc.modelName && (
                        <InfoItem>
                          <strong>Model:</strong> {doc.modelName}
                        </InfoItem>
                      )}
                      {doc.imei1 && (
                        <InfoItem>
                          <strong>IMEI 1:</strong> {doc.imei1}
                        </InfoItem>
                      )}
                      {doc.imei2 && (
                        <InfoItem>
                          <strong>IMEI 2:</strong> {doc.imei2}
                        </InfoItem>
                      )}
                      {doc.salePrice && (
                        <InfoItem>
                          <strong>Sale Price:</strong> Rs {doc.salePrice}
                        </InfoItem>
                      )}
                      {doc.totalInvoice && (
                        <InfoItem>
                          <strong>Total Invoice:</strong> Rs {doc.totalInvoice}
                        </InfoItem>
                      )}
                      {doc.sellingPaymentType && (
                        <InfoItem>
                      ...
                      {doc.sellingPaymentType === 'Credit' && (
                        <>
                          {doc.payableAmountNow !== undefined && (
                            <InfoItem>
                              <Dot color="green" />
                              <strong> Pay Now:</strong> Rs{' '}
                              {doc.payableAmountNow}
                            </InfoItem>
                          )}
                          {doc.payableAmountLater !== undefined && (
                            <InfoItem>
                              <Dot color="red" />
                              <strong> Pay Later:</strong> Rs{' '}
                              {doc.payableAmountLater}
                            </InfoItem>
                          )}
                        </>
                      )}
                      ...
                      {doc.warranty && (
                        <InfoItem>
                          <strong>Warranty:</strong> {doc.warranty}
                        </InfoItem>
                      )}
                      {doc.specifications && (
                        <InfoItem>
                          <strong>Specifications:</strong> {doc.specifications}
                        </InfoItem>
                      )}
                      {doc.phoneCondition && (
                        <InfoItem>
                          <strong>Condition:</strong> {doc.phoneCondition}
                        </InfoItem>
                      )}
                      {doc.createdAt && (
                        <InfoItem>
            ...
            {soldRecord
              ...
              .map((record, index) => {
                ...
                const isBulkPurchase =
                  doc.ramSimDetails && Array.isArray(doc.ramSimDetails);

                return (
                  <Card key={doc._id || index} type="purchase">
                    <CardHeader>
                      {isBulkPurchase ? 'Bulk Purchase' : 'Purchased Phone'}
                    </CardHeader>
                    <InfoGrid>
                      {/* Party/Person information */}
                      {doc.personId?.name && (
                        <InfoItem>
                          <strong>Party Name:</strong> {doc.personId.name}
                        </InfoItem>
                      )}
                      {doc.name && (
                        <InfoItem>
                          <strong>Customer Name:</strong> {doc.name}
                        </InfoItem>
                      )}
                      ...
                      {isBulkPurchase ? (
                        <>
                          ...
                          {doc.purchasePaymentStatus && (
                            <InfoItem>
                              <strong>Payment Status:</strong>{' '}
                              {doc.purchasePaymentStatus}
                            </InfoItem>
                          )}
                          {doc.purchasePaymentType && (
                            <InfoItem>
                              <strong>Payment Type:</strong>{' '}
                              {doc.purchasePaymentType}
                            </InfoItem>
                          )}
                          {doc.prices?.buyingPrice && (
                            <InfoItem>
                              <strong>Buying Price:</strong> Rs{' '}
                              {doc.prices.buyingPrice}
                            </InfoItem>
                          )}
                          {doc.prices?.dealerPrice && (
                            <InfoItem>
                          ...
                          {doc.ramSimDetails?.map((ramSim, ramIndex) => (
                            ...
                            >
                              <InfoItem>
                                <strong>Model {ramIndex + 1}:</strong>{' '}
                                {ramSim.modelName || 'Unknown'}
                              </InfoItem>
                              <InfoItem>
                                <strong>Company:</strong>{' '}
                                {ramSim.companyName || 'Unknown'}
                              </InfoItem>
                              <InfoItem>
                                <strong>RAM:</strong>{' '}
                                {ramSim.ramMemory || 'N/A'}
                              </InfoItem>
                              <InfoItem>
                                <strong>SIM:</strong>{' '}
                              ...
                              <InfoItem>
                                ...
                                {ramSim.priceOfOne || 'N/A'}
                              </InfoItem>

                              {/* IMEI Numbers */}
                              {ramSim.imeiNumbers?.map((imei, imeiIndex) => (
                                <InfoItem key={imeiIndex}>
                                  <strong>IMEI {imeiIndex + 1}:</strong>{' '}
                                  {imei.imei1}
                                  {imei.color && (
                                    <span> (Color: {imei.color})</span>
                                  )}
                                </InfoItem>
                              ))}
                            </div>
                        ...
                        <>
                          ...
                          {doc.price?.purchasePrice && (
                            <InfoItem>
                              <strong>Purchase Price:</strong> Rs{' '}
                              {doc.price.purchasePrice}
                            </InfoItem>
                          )}
                          {doc.price?.finalPrice && (
                            <InfoItem>
                              <strong>Final Price:</strong> Rs{' '}
                              {doc.price.finalPrice}
                            </InfoItem>
                          )}
                          {doc.price?.demandPrice && (
                            <InfoItem>
                              <strong>Demand Price:</strong> Rs{' '}
                              {doc.price.demandPrice}
                            </InfoItem>
                          )}
                          {doc.color && (
                          ...
                          {doc.ramMemory && (
                            <InfoItem>
                              <strong>RAM:</strong> {doc.ramMemory}
                            </InfoItem>
                          )}
                          {doc.warranty && (
                            <InfoItem>
                    ...
                    {!isBulkPurchase &&
                      doc.accessories &&
                      typeof doc.accessories === 'object' && (
                        <div style={{ marginTop: '12px' }}>
                          <strong>Accessories:</strong>
                          <AccessoriesList>
                            {Object.entries(doc.accessories).map(
                              ([key, value]) => (
                                <li key={key}>
                                  {key}: {value ? 'Yes' : 'No'}
                                </li>
      ...
      >
        ...
        >
          <div style={{ width: '100%' }}>
            ...
            >
              Choose a person and optional date range to load sales and
              purchases.
            </p>
          </div>
          {/* <select
          ...
          <select
            ...
            onBlur={(e) => {
              ...
              e.target.style.boxShadow = '0 2px 8px rgba(17,24,39,0.06)';
            }}
            required
          >
            ...
            >
              <button
                ...
                style={{
                  ...
                  fontWeight: 600,
                  background:
                    'linear-gradient(135deg,#94a3b8 0%, #6b7280 100%)',
                  color: '#fff',
                  border: 'none',
                  borderRadius: '10px',
                  cursor: 'pointer',
