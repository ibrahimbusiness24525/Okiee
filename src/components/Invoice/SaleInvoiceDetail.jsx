...
const SaleInvoiceDetail = () => {
  ...
  const normalizePhones = () => {
    ...
    // Handle imeiDetails array for bulk sales
    if (
      rawPhoneDetails?.imeiDetails &&
      Array.isArray(rawPhoneDetails.imeiDetails)
    ) {
      return rawPhoneDetails.imeiDetails.map((imeiDetail) => ({
        companyName: rawPhoneDetails?.companyName || invoice?.companyName || '',
        modelName: rawPhoneDetails?.modelName || invoice?.modelName || '',
        imei: imeiDetail?.imei1 || '',
        imei2: imeiDetail?.imei2 || '',
        ramMemory: imeiDetail?.ramMemory || rawPhoneDetails?.ramMemory || '',
        color: imeiDetail?.color || rawPhoneDetails?.color || '',
        phoneCondition: rawPhoneDetails?.phoneCondition || '',
        warranty: rawPhoneDetails?.warranty || '',
        salePrice:
          toNumber(imeiDetail?.priceOfOne) || toNumber(pricing?.salePrice) || 0,
      }));
    }
    ...
    // If no phone details but we have pricing, create at least one entry
    if (
      detailsArray.length === 0 &&
      (pricing?.salePrice > 0 || pricing?.totalInvoice > 0)
    ) {
      const totalSalePrice =
        toNumber(pricing?.salePrice) || toNumber(pricing?.totalInvoice) || 0;
      phones.push({
        companyName: rawPhoneDetails?.companyName || invoice?.companyName || '',
        modelName: rawPhoneDetails?.modelName || invoice?.modelName || '',
        imei: Array.isArray(rawPhoneDetails?.imei1)
          ? rawPhoneDetails.imei1.join(', ')
          : rawPhoneDetails?.imei1 || '',
        imei2: Array.isArray(rawPhoneDetails?.imei2)
          ? rawPhoneDetails.imei2.join(', ')
          : rawPhoneDetails?.imei2 || '',
        ramMemory: rawPhoneDetails?.ramMemory || '',
        color: rawPhoneDetails?.color || '',
        phoneCondition: rawPhoneDetails?.phoneCondition || '',
        warranty: rawPhoneDetails?.warranty || '',
    ...
    detailsArray.forEach((detail) => {
      ...
      const imei2List = splitMaybeList(detail?.imei2 ?? rawPhoneDetails?.imei2);
      ...
      for (let i = 0; i < count; i++) {
        ...
        const fallbackUnit =
          ...
          0;
        const salePrice =
          imei && imeiPriceMap[imei] !== undefined
            ? imeiPriceMap[imei]
            : fallbackUnit;
        ...
        phones.push({
          ...
          imei,
          imei2,
          ramMemory: detail?.ramMemory || rawPhoneDetails?.ramMemory || '',
          color: detail?.color || rawPhoneDetails?.color || '',
          phoneCondition:
            detail?.phoneCondition || rawPhoneDetails?.phoneCondition || '',
          warranty: detail?.warranty || rawPhoneDetails?.warranty || '',
  ...
  // For generic sales, if phonesTotalComputed is 0 but we have pricing.salePrice, use that
  // For generic sales, prioritize pricing.salePrice if phonesTotalComputed is 0
  const phonesTotal =
    phonesTotalComputed > 0
      ? phonesTotalComputed
      : invoice?.saleType === 'generic'
        ? toNumber(pricing?.salePrice) || toNumber(pricing?.totalInvoice) || 0
        : toNumber(pricing?.salePrice) ||
